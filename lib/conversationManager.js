...
export async function compressHistory(messages) {
  ...
  try {
    ...
    const { logCostViaApi }         = await import('./vmCostLogger.js');
    ...
    logCostViaApi(summary.modelKey, summary.usage, 'conversation_compress');
    ...
    console.error('[conversationManager:compressHistory]', err.message);
    return toKeep;
  }
}
