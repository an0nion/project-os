...
import { APP_TZ } from './timezone.js';

// Lazy supabase loader — only needed for token storage (token ops are Vercel-only,
// never called on the VM). pickColorId has no DB dependency.
let _supabase = null;
async function db() {
  if (!_supabase) ({ supabase: _supabase } = await import('./supabase.js'));
  return _supabase;
}
...
export const COLOR = {
  ...
  APPOINTMENTS:  '3',   // Grape      — bright purple
  WORK:          '9',   // Blueberry  — dark blue
  EVENTS:        '4',   // Flamingo   — salmon pink  (events + conferences + talks)
  BIRTHDAYS:     '5',   // Banana     — yellow
  TODO:          '2',   // Sage       — bright green
  SCHOOL_TASKS:  '10',  // Basil      — dark green
  GRADED:        '1',   // Lavender   — light purple
  OPTIONAL:      '8',   // Graphite   — grey (truly optional only)
};
...
export function pickColorId(project, text) {
  ...
  if (/\bdoctor|dentist|physio|\bgp\b|appointment|outing|catch.?up/.test(t)) return COLOR.APPOINTMENTS;
  if (/\bcancel|subscription|renew|expires?|warning\b/.test(t))              return COLOR.WARNINGS;
  if (/\bconference|neurips|icml|iclr|\bnips\b|symposium|seminar|talk\b|info session|event/.test(t)) return COLOR.EVENTS;
  if (/\boptional/.test(t))                                                  return COLOR.OPTIONAL;

  switch (project) {
    case 'work':          return COLOR.WORK;
    case 'school':        return COLOR.SCHOOL_TASKS;
    case 'personal':      return COLOR.TODO;
    case 'research_apps': return COLOR.EVENTS;
...
export async function storeTokens({ access_token, refresh_token, expires_in, existing_refresh }) {
  ...
  await (await db()).from('calendar_tokens').upsert({
    user_id:       TOKEN_KEY,
    access_token,
...
// ── Add N minutes to a local datetime string (no UTC conversion) ─────────────
// The VM runs UTC. new Date(localString).getTime() treats the string as UTC,
// creating an AEST offset. We manipulate the string directly to stay local.
function addMinutesLocal(dt, minutes) {
  ...
  const dayRoll = Math.floor(totalMins / 60) >= 24;
  if (dayRoll) {
    const d = new Date(datePart + 'T00:00:00Z');
    d.setUTCDate(d.getUTCDate() + Math.floor(totalMins / (60 * 24)));
    return `${d.toISOString().slice(0, 10)}T${String(newH).padStart(2, '0')}:${String(newM).padStart(2, '0')}:${s}`;
  }
  return `${datePart}T${String(newH).padStart(2, '0')}:${String(newM).padStart(2, '0')}:${s}`;
}

// ── Create a Google Calendar event ────────────────────────────────────────────
// date:             'YYYY-MM-DD' for all-day, ISO datetime string for timed events
// colorId:          from COLOR map above — sets per-event colour
// description:      shown in event notes field
// durationMinutes:  event length in minutes (default 60, ignored for all-day events)
// reminderMinutes:  array of minutes-before for popup reminders (default [30])
//                   empty array = no reminders; null = use Google's default
export async function createCalendarEvent({
  ...
  if (isAllDay) {
    ...
    end   = { dateTime: addMinutesLocal(date, durationMinutes), timeZone: tz };
  }
