...
export async function createCalendarEvent({ title, date, colorId, description }) {
  ...
  if (isAllDay) {
    ...
    // Add 1 hour to the local datetime string directly — no UTC conversion.
    // new Date(localString).getTime() treats the string as UTC on the VM (which runs UTC),
    // creating an 11-hour offset against Melbourne local time. Instead, parse the hour
    // component from the string and increment it, keeping the date string intact.
    const addOneHourLocal = (dt) => {
      const [datePart, timePart] = dt.split('T');
      const [h, m, s = '00'] = timePart.split(':');
      const newH = (parseInt(h) + 1) % 24;
      const rollDay = parseInt(h) + 1 >= 24;
      if (rollDay) {
        const d = new Date(datePart + 'T00:00:00Z');
        d.setUTCDate(d.getUTCDate() + 1);
        return `${d.toISOString().slice(0, 10)}T00:${m}:${s}`;
      }
      return `${datePart}T${String(newH).padStart(2, '0')}:${m}:${s}`;
    };
    start = { dateTime: date, timeZone: tz };
    end   = { dateTime: addOneHourLocal(date), timeZone: tz };
  }
